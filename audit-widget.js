...
/* ====== CONFIG ====== */
const CONFIG = {
  // Paste the Apps Script /exec URL here after you redeploy google-sheets-setup.gs
  GOOGLE_SHEETS_URL: "https://script.google.com/macros/s/AKfycbz4QX4tDAxxSzPHcGSk-BypRsAv2WuSn1Twkurj_yn0EXhAU7oQrac6mmYhejYrLr8v/exec",
  WHATSAPP_NUMBER: "917654925271",
  SCALE_URL: "scale.html",
  // Same Supabase project used by scale.html, so all leads live in one place
  ...
  SUPABASE_AUDIT_TABLE: "audit_leads"
};
...
function renderFree(screen){
  app.innerHTML = `
    <div class="screen">
      ...
      <h2 class="qtext">${screen.data.text}</h2>
      <textarea class="input-text" id="freeInput" rows="3" placeholder="Type as much or as little as you like\u2026" aria-label="${escapeAttr(screen.data.text)}" style="resize:none;overflow:hidden;" oninput="this.style.height='auto';this.style.height=this.scrollHeight+'px';"></textarea>
      ${navRow(true)}
    </div>`;
...
function fitScoreNum(el){
  if(!el) return;
  const max = 60, min = 30;
  let size = max;
  el.style.fontSize = size + "px";
  const cap = el.closest(".score-hero") || el.parentElement;
  while(el.scrollWidth > cap.clientWidth && size > min){
    size -= 2;
    el.style.fontSize = size + "px";
  }
}
...
function renderResult(){
  ...
  const catRows = Object.keys(CATEGORY_META).map(c=>`
    <div class="cat-bar-row">
      ...
      <div class="cat-bar-track"><div class="cat-bar-fill" data-w="${r.catScores[c]*10}" style="width:0%"></div></div>
  ...
  app.innerHTML = `
    <div class="screen" style="padding-top:24px;">
      <div class="score-hero">
        ...
        <div class="score-num" id="scoreNum" data-target="${r.overall}">0<span>/10</span></div>
  ...
  requestAnimationFrame(()=>{
    ...
    if(numEl){
      ...
      (function tick(now){
        ...
        numEl.firstChild.textContent = target % 1 === 0 ? Math.round(val) : val.toFixed(1);
        if(span) numEl.appendChild(span);
        if(p < 1) requestAnimationFrame(tick); else { numEl.firstChild.textContent = target; fitScoreNum(numEl); }
      })(start);
      fitScoreNum(numEl);
    }
    document.querySelectorAll(".cat-bar-fill").forEach((el,i)=>{
...
function submitToSheets(r){
  ...
  submitToSupabase(r); // independent of Sheets — runs even if Sheets URL isn't configured yet
...
/* ============================= SUPABASE SUBMIT ============================= */
// Writes the same completed-audit lead into Supabase, independent of the
// Sheets write above. Fire-and-forget — a failure here never blocks or
// delays the result screen the user is already looking at.
function submitToSupabase(r){
  ...
  const payload = {
    source: state.src || "audit",
    vertical: state.vertical || "",
    business_type: state.verticalLabel || "",
    custom_business: state.customBusiness || "",
    business_name: state.answers["leadBusiness"] || "",
    person_name: state.answers["leadName"] || "",
    city: state.answers["leadLocation"] || "",
    whatsapp: state.answers["leadPhone"] || "",
    overall_score: r.overall,
    tier: r.tier,
    category_scores: r.catScores,
    recommended_tools: r.rankedTools,
    final_thought: (state.answers["final"] && state.answers["final"].label) || "",
    context: context
  };

  fetch(CONFIG.SUPABASE_URL + "/rest/v1/" + CONFIG.SUPABASE_AUDIT_TABLE, {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify(payload)
  }).catch(()=>{ /* fail silently — Sheets write above is the backup */ });
}
